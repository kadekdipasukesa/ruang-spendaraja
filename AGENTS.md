...
* `src/components/RuangBelajar/TugasKhusus/Tugas3/`: Petualangan 4 Misi Sistem Komputer & Perkakas Digital:
  ...
  - Fitur Persistensi & Sinkronisasi: Kebijakan **Database-First Priority** dengan sinkronisasi ke tabel `tugas_pengumpulan`, `point_logs`, dan `master_siswa.total_points`. Kueri database diselaraskan secara akurat dengan skema nyata (menggunakan kolom `skor` tanpa kolom usang `nilai_akhir`) dan penyaringan UUID ketat (`isValidUUID`). Posisi draft penempatan komponen dicadangkan ke database pada saat tombol "Kumpulkan Tugas" ditekan (dalam kolom `detail_jawaban.placements`) sehingga siswa dapat melanjutkan tugas di perangkat berbeda. Saat membuka atau melanjutkan tugas ("Lanjutkan Petualangan"), sistem memprioritaskan data dari database di atas cache lokal, menginjeksi snapshot `placements` langsung via prop `initialPlacements` ke semua komponen misi, serta secara cerdas mengarahkan navigasi langsung ke misi yang belum terselesaikan. Proteksi nilai database menjamin nilai resmi di database tidak ditimpa/di-replace jika nilai pengerjaan baru lebih kecil, disertai dialog pemberitahuan proteksi skor yang transparan.
