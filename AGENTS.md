...
10. **[`/docs/LIVE_CHAT.md`](/docs/LIVE_CHAT.md)**: Dokumentasi teknis & arsitektur `LiveChat.jsx` (skema ternormalisasi relasional `sender_id` terhubung `master_siswa`, arsitektur modular `ChatHeader`, `ChatMessageItem`, `ChatInputForm`, `chatHelpers`, `LinkPreviewCard`, Supabase Realtime Postgres Changes, moderasi sensor kata kasar, sistem pemisahan 2 ruang terisolasi [Ruang Siswa vs. Ruang Guru], tab switcher & 3 lapis proteksi anti-salah kamar untuk Admin, kontrol kunci chat kelas, audio SFX, smart auto-scroll, rendering tautan anti-tembus, sistem kartu link preview modular ala WhatsApp dengan oEmbed TikTok & Instagram embed scraper tanpa login, serta mekanisme *Exclusive Video Playback* anti-tumpang tindih suara).
