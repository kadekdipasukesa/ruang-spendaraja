...
export default function RepresentationQuizPanel({
  questions,
  answers,
  submitted,
  scoreResult,
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      <div className="text-center space-y-1">
        <span className="text-xs font-bold text-purple-400 uppercase tracking-wider flex items-center justify-center gap-1.5">
          <HelpCircle className="w-4 h-4" /> Uji Pemahaman: Representasi Data (2 Kemungkinan Ya/Tidak)
        </span>
        <p className="text-[11px] text-slate-400">
          Pilihlah opsi yang paling tepat. Pilihan jawaban telah disusun dengan panjang kalimat seimbang dan teracak.
        </p>
      </div>
          ...
          return (
            <div
              ...
              className={`p-4 sm:p-5 rounded-2xl border transition-all ${
                selectedOptionId
                  ? 'bg-slate-900/90 border-purple-500/40 shadow-xs'
                  : 'bg-slate-950 border-slate-800'
              }`}
            >
              <div className="flex items-start gap-2.5 mb-3">
                <span className="px-2 py-0.5 rounded-md bg-purple-500/20 text-purple-300 font-mono text-xs font-bold shrink-0 mt-0.5">
                  #{qIdx + 1}
                </span>
      ...
      <div className="p-4 sm:p-5 rounded-2xl bg-slate-950 border border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        {!submitted ? (
          <button
            ...
            className="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-gradient-to-r from-purple-500 to-pink-600 hover:from-purple-400 hover:to-pink-500 text-white font-bold text-xs transition shadow-lg shadow-purple-500/25 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer"
          >
            <Sparkles className="w-4 h-4" /> Kumpulkan & Lihat Hasil Nilai
          </button>
        ) : (
          <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
            <div className="px-4 py-2.5 rounded-xl bg-purple-500/20 border border-purple-500/40 text-purple-300 flex items-center gap-2.5">
              <Award className="w-5 h-5 text-purple-400 shrink-0" />
              <div>
                <div className="text-[10px] text-purple-400 uppercase font-bold">Hasil Skor Kuis</div>
