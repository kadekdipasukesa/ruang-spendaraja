import { 
  ...
  Trophy, Lock, BookOpen, UserCheck 
} from 'lucide-react';
...
export default function Navigation({ activeTab, setActiveTab, student }) {
  const tabs = [
    ...
    { id: 'absen', label: 'Absensi', icon: <UserCheck size={18} /> }, // Menu Absen Baru
  ...
  const handleTabClick = (tabId) => {
    // Absen sekarang masuk ke daftar yang diproteksi (harus login)
    const isProtected = tabId === 'tugas' || tabId === 'ranking' || tabId === 'absen';
    ...
    <div className="flex gap-2 bg-slate-900/50 p-1.5 rounded-2xl mb-8 border border-white/5 overflow-x-auto no-scrollbar">
      {tabs.map((tab) => (
        ...
        >
          ...
          {/* Tampilkan icon gembok jika belum login pada menu tertentu */}
          {(tab.id === 'tugas' || tab.id === 'ranking' || tab.id === 'absen') && !student && (
